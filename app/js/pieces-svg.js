/**
 * Inline vector chess pieces -- the "Cburnett" set, the same artwork Wikipedia uses for its
 * chess diagrams and lichess.org ships as its default board theme. Shipping the *authentic*
 * set (rather than a hand-drawn lookalike) means piece recognition transfers from those sites
 * instead of having to be re-learned here.
 *
 * Each piece is exported as raw inline SVG markup (a fragment of <path>/<g> elements, no
 * outer <svg> wrapper) authored in Cburnett's native 45x45 unit box -- PIECE_BOX below --
 * so board2d.js can drop it into a board cell with a single scale+translate transform, same
 * as before. The markup is renderer-agnostic: any consumer that injects it into an SVG
 * document (see cloneMarkup() in board2d.js) gets crisp vector output at any DPR.
 *
 * Unlike the previous hand-authored silhouettes, this artwork carries its own baked-in
 * colours -- white pieces are white-filled with black outlines, black pieces are black-filled
 * with light-grey (#ececec) detail strokes -- which is the standard, immediately-recognisable
 * look. That is the entire point of using the authentic set, so this module does not expose
 * a recolour hook the way the old body/detail path arrays did; board2d.js no longer maps
 * --piece-white / --piece-black onto piece fills (those tokens remain in use for the rest of
 * the app -- see app/js/board3d.js and app/css/tokens.css).
 *
 * Source: lichess-org/lila, public/piece/cburnett/*.svg
 * (https://github.com/lichess-org/lila/tree/master/public/piece/cburnett), which redistributes
 * Colin M.L. Burnett's original Wikimedia Commons artwork (e.g.
 * https://commons.wikimedia.org/wiki/File:Chess_kdt45.svg) unmodified in shape.
 *
 * Licence: Creative Commons Attribution-ShareAlike 3.0 Unported (CC BY-SA 3.0), one of several
 * licences Burnett released the set under -- see app/vendor/pieces/LICENSE.md for the full
 * attribution, licence text pointers, and the vendored source SVGs this module's markup was
 * taken from verbatim.
 */

export const PIECE_BOX = 45;

/** Short credit line for any About/Settings screen to render. */
export const PIECE_CREDIT =
  'Chess pieces: "Cburnett" set by Colin M.L. Burnett, CC BY-SA 3.0.';

// Piece markup keyed by full piece code ('w'|'b' + K/Q/R/B/N/P), since -- unlike the old
// silhouettes -- colour is baked into the artwork rather than applied by the renderer.
export const PIECES = {
...
/** Full piece codes ('wK', 'bP', ...) this module supplies markup for. */
